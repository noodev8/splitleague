...
- [x] **Build the share slug** — done 2026-08-29. `league.share_slug`, ten characters of Crockford base32, generated once and never rotated. Full handover in `docs/next-share-slug.md`.
  ...
  **Deployed and finished 2026-08-30.** The NOT NULL had been deliberately relaxed while the deploy was pending — the old `create_league` did not write a slug, so the constraint would have stopped anyone creating a league in that window. It is back on now. No leagues were created during the gap (0 nulls, no backfill needed), and crucially the *deployed* `create_league` was proved to write a well-formed slug **before** the constraint went back, then again with it enforced; both throwaway leagues were deleted. A create path that silently does not write the column is exactly what would have turned NOT NULL into an outage. Final state: 192 leagues, 192 slugs, all distinct and well formed.
