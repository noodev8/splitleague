...
router.post('/', verifyToken, async (req, res) => {
  try {
    ...
    let standings = members.map(member => {
      ...
      // Add specific stats based on league type
      if (winType === 'WDL') {
        playerStats.drawn = 0;
      }
    ...
    fixtures.forEach(fixture => {
      ...
      if (winType === 'PTS') {
        ...
        if (fixture.player_1_score > fixture.player_2_score) {
          ...
          // Draw (equal scores)
          standings[player1Index].drawn = (standings[player1Index].drawn || 0) + 1;
          standings[player2Index].drawn = (standings[player2Index].drawn || 0) + 1;
